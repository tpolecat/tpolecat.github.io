...
```scala
scala> sql"select * from person where id = $pid"
res10: doobie.syntax.string.SqlInterpolator#Builder = doobie.syntax.string$SqlInterpolator$Source@22b66f72
...
```scala
scala> implicit val JsonMeta = Meta.other[PGobject]("json").nxmap[Json](
     |     a => Parse.parse(a.getValue).leftMap[Json](sys.error).merge,
     |     a => new PGobject <| (_.setType("json")) <| (_.setValue(a.nospaces)))
JsonMeta: doobie.util.meta.Meta[argonaut.Json] = doobie.util.meta$Meta$$anon$2@6d5c4c7e
```
...
```scala
scala> def codecMeta[A >: Null : CodecJson: TypeTag] =
     |   Meta[Json].nxmap[A](
     |     _.as[A].result.fold(p => sys.error(p._1), identity), 
     |     _.asJson
     |   )
codecMeta: [A >: Null](implicit evidence$1: argonaut.CodecJson[A], implicit evidence$2: reflect.runtime.universe.TypeTag[A])doobie.util.meta.Meta[A]
```
...
```scala
scala> sql"select 12, 42".query[Point].unique.quick.run
  java.awt.Point[x=12,y=42]
```
