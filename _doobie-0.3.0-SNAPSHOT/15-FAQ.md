...
scala> sql"select $s".query[String].check.unsafePerformSync
...
scala> sql"select $s :: char".query[String].check.unsafePerformSync
...
```
scala> cities(Code("USA"), true).check.unsafePerformSync
...
```scala
scala> cities(Code("USA"), true).process.take(5).quick.unsafePerformSync
...
scala> cities(Code("USA"), false).process.take(5).quick.unsafePerformSync
...
```scala
scala> join.process.filter(_._1.name.startsWith("United")).quick.unsafePerformSync
...
<console>:32: error: Could not find or construct Param[shapeless.::[String,shapeless.::[java.util.UUID,shapeless.HNil]]].
...
```
scala> Meta[UUID]
<console>:32: error: Could not find an instance of Meta[java.util.UUID]; you can construct one based on a primitive instance via `xmap`.
       Meta[UUID]
           ^
```
...
<console>:33: error: Could not find or construct Composite[java.awt.geom.Point2D.Double].
Ensure that this type has a Composite instance in scope; or is a Product type whose members have
Composite instances in scope; or is an atomic type with an Atom instance in scope. You can usually
diagnose this problem by trying to summon the Composite instance for each element in the REPL. See
the FAQ in the Book of Doobie for more hints.
