...
<console>:44: error: Could not find or construct Param[shapeless.::[PersonId,shapeless.HNil]].
...
```scala
scala> Meta[PersonId]
<console>:41: error: Could not find an instance of Meta[PersonId]; you can construct one based on a primitive instance via `xmap`.
       Meta[PersonId]
           ^
```
...
```scala
scala> sql"select * from person where id = $pid"
res18: doobie.syntax.string.Builder[shapeless.::[PersonId,shapeless.HNil]] = doobie.syntax.string$Builder@4629a57c
...
scala> sql"select owner from pet".query[Int].check.unsafePerformIO
  ...
  ✕ C01 owner OTHER (json) NOT NULL  →  Int
    - OTHER (json) is not coercible to Int according to the JDBC specification or any
      ...
      type to Json or PGobject or Person.
```
