...
```scala
scala> (drop.run *> create.run).transact(xa).unsafePerformIO
res1: Int = 0
```
...
```scala
scala> insert1("Alice", Some(12)).run.transact(xa).unsafePerformIO
res2: Int = 1

scala> insert1("Bob", None).quick.unsafePerformIO // switch to YOLO mode
  1 row(s) updated
```
